...
def main():
    ...
    src_ddl = """
        CREATE TABLE salesitems (
            ...
            'properties.group.id' = 'session-windows',
            'format' = 'json'
        )
    """
    ...
    #
    # - Calculate total revenue for windows separated by periods of 
    # - inactivity (no orders) for at least 20 seconds
    #####################################################################
    windowed_rev = tbl.window(Session.with_gap(lit(20).seconds)
                              .on(tbl.proctime)
                              .alias('w'))\
